...
export default function AdminReferralPage() {
  ...
  return <AdminPageShell title="Admin Referrals" description="Pantau attribution, status referral, payment, dan komisi terkait.">
    ...
    <Filters onRefresh={() => void load()}><AdminSelect value={status} onChange={(e) => setStatus(e.target.value)}><option value="all">Semua status</option><option value="registered">Registered</option><option value="trial_started">Trial</option><option value="paid">Paid</option><option value="eligible">Eligible</option><option value="rewarded">Rewarded</option><option value="rejected">Rejected</option><option value="cancelled">Cancelled</option></AdminSelect><AdminSelect value={type} onChange={(e) => setType(e.target.value)}><option value="all">Semua tipe</option><option value="customer_referral">Customer referral</option><option value="affiliate">Affiliate</option></AdminSelect><AdminInput value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari referral" /><button onClick={() => void load()} className="rounded-2xl bg-[#FF6A00] px-4 py-3 text-sm font-black text-white">Cari</button></Filters>

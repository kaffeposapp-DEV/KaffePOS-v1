...
import { AdminInput, AdminPageShell, AdminSelect, DetailModal, EmptyState, ErrorCard, Filters, LoadingCard, StatusBadge, SummaryCards, money } from './adminUi';
...
export default function AdminAffiliatePage() {
  ...
  return <AdminPageShell title="Admin Affiliates" description="Kelola status affiliate, payout masked, dan performa partner.">
    ...
    <Filters onRefresh={() => void load()}><AdminSelect value={status} onChange={(e) => setStatus(e.target.value as AffiliateStatus | 'all')}><option value="all">Semua status</option><option value="pending">Pending</option><option value="active">Active</option><option value="suspended">Suspended</option><option value="rejected">Rejected</option></AdminSelect><AdminInput value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari email/kode" /><button onClick={() => void load()} className="rounded-2xl bg-[#FF6A00] px-4 py-3 text-sm font-black text-white">Cari</button></Filters>
